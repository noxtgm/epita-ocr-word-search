    ...
    for (int r = 0; r < num_rows; r++) {
        ...
        if (best_anchor_length >= 3) {
            ...
            for (int j = best_anchor_start + 1; j < row_counts[r]; j++) {
                int idx_j = row_chars[r][j];
                float spacing = (float)(chars[idx_j].center_x - chars[last_kept_idx].center_x);
                float diff = fabs(spacing - best_spacing);
...
void draw_rect_green(Image* img, int x1, int y1, int x2, int y2) {
    ...
    for (int x = x1; x <= x2; x++) {
        set_cached_pixel(img, x, y1, 0, 0);
        set_cached_pixel(img, x, y1, 1, 255);
        set_cached_pixel(img, x, y1, 2, 0);
...
void draw_rect(Image* img, int x1, int y1, int x2, int y2) {
    ...
    for (int x = x1; x <= x2; x++) {
        set_cached_pixel(img, x, y1, 0, 255);
        set_cached_pixel(img, x, y1, 1, 0);
        set_cached_pixel(img, x, y1, 2, 0);
...
void morphological_closing(unsigned char **binary, int width, int height, int iterations) {
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int y = 0; y < height; y++) {
            memcpy(binary[y], temp[y], width);
            free(temp[y]);
        }
        free(temp);
        
        temp = malloc(height * sizeof(unsigned char*));
        for (int y = 0; y < height; y++) {
            temp[y] = calloc(width, sizeof(unsigned char));
        }
        ...
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                if (binary[y][x]) {
                    ...
                    if (binary[y-1][x]) neighbor_count++;
                    if (binary[y+1][x]) neighbor_count++;
                    if (binary[y][x-1]) neighbor_count++;
                    if (binary[y][x+1]) neighbor_count++;
                    
                    temp[y][x] = (neighbor_count >= 2) ? 1 : 0;
                }
            }
        }
...
int detect_list(Image *img, DetectionData* data, int x1, int y1, int x2, int y2, 
    ...
    for (int y = 0; y < img->height; y++) {
        for (int x = 0; x < img->width; x++) {
            int gray = (int)(0.3 * get_cached_pixel(data->work_img, x, y, 0) + 
                            0.59 * get_cached_pixel(data->work_img, x, y, 1) + 
                            0.11 * get_cached_pixel(data->work_img, x, y, 2));
            data->binary[y][x] = (gray < 185.78) ? 1 : 0;
        }
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (Lan) {
            if (b->pixel_count < 30 || b->minx <= 54 || 
                (b->minx > 200 && b->minx < 270 && b->miny < y1 - 40)) {
                continue;
            }
        }
    ...
    for (int w = 0; w < row_count; w++) {
        ...
        int rxl = 0;
        if (Lan) {
            if (word_h > 80 || word_w > 150 || word_h <= 13)
                rxl = 1;
        }
